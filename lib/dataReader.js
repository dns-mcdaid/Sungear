...
const Vessel = require('../public/javascripts/genes/vessel');
            ...
            speciesCol.find({'name': species}).toArray(function (err, result) {
                ...
                experimentsCol.find({
                    ...
                    itemsCol.find({'species': thisSpecies}).toArray(function(err, result) {
                        ...
                        items = result.filter(function(gene) {
                        ...
                        categoriesCol.find({'species': thisSpecies}).toArray(function(err, result) {
                            ...
                            console.log(result.length);
                            categories = result.filter(function(category) {
            ...
            speciesCol.find({'name': species}).toArray(function(err, result) {
                ...
                itemsCol.find({'species': thisSpecies}).toArray(function(err, result) {
                    ...
                    categoriesCol.find({'species': thisSpecies}).toArray(function(err, result) {
                        ...
                        //             var gn = term.items[j];
                        //             var g = allGenes[gn.toLowerCase()];
                        //             if (g === null || typeof g === 'undefined') {
                        //                 missingGene.push(gn);
                        //             } else {
                        //                 var genev = geneToGo[g];
                        //                 if (typeof genev === 'undefined' || genev === null) {
                        //                     genev = [];
                        //                 }
                        //                 genev.push(termObject.id);
                        ...
                        experimentsCol.find({
                            ...
                            //                 var contains = lists[setNames[k]].indexOf(thisList[j]);
                            //                 if (contains > -1) {
                            //                     exp[k] = 1;
                            //                 } else {
                            //                     exp[k] = 0;
